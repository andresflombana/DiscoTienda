...
public class Discotienda
{
    ...
    /**
     * Método para generar un reporte de canciones
    ...
    public void generarInformeDiscosCostosos() throws FileNotFoundException, Exception {
        File miArchivo = new File("./data/discosCostosos.txt");
        PrintWriter pluma = new PrintWriter(miArchivo);
        
        pluma.println("----Informe de Discos Costosos----");
        pluma.println("-----------------------------------");
        
        boolean hayDiscosCostosos = false;

        for (int i = 0; i < discos.size(); i++) {
            Disco miDisco = (Disco) discos.get(i);
            // Convertir el género a mayúsculas
            String genero = miDisco.darGenero().toUpperCase();
            
            // Comprobación del género y del precio
            if ((genero.equals("ROCK") || genero.equals("POP")) || miDisco.darPrecioDisco() > 3500) {
                hayDiscosCostosos = true;
                pluma.println("Nombre: " + miDisco.darNombreDisco());
                pluma.println("Artista: " + miDisco.darArtista());
                pluma.println("Género: " + miDisco.darGenero());
                pluma.println("Precio: " + miDisco.darPrecioDisco());
        ...
        if (!hayDiscosCostosos) {
            throw new Exception("No se encontraron discos de los géneros ROCK o POP con un precio superior a $3,500.");
        }
    }
